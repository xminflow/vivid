...
**选项值三处同步。** `constants.ts` 里的来者身份、预约需求、服务类别，必须与
...
**删除线索是硬删除。** 两个列表页的「删除」直接把记录从库里删掉，没有软删除、没有
回收站，只在点击时用 `confirm()` 做一次二次确认。这么定是因为运营要的就是「处理完的
线索从列表里消失」；做成软删除会多出一个「已删除」筛选项和一堆没人看的存量。
服务申请连着的客户图片不跟着删，理由见 `server/app/admin.py`。
...
- 导出、商品模块。（状态流转不做了：`status` 列已经删掉，改用「删除」清理线索。）
