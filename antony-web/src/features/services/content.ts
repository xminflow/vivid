/**
 * 五个服务的文案与申请表单定义。与小程序 mock/service.js 是同一份内容，改动要两边同步。
 *
 * 表单是数据驱动的：ApplyPage 按 fields 数组渲染，加字段改这里就行，不用动页面。
 * uploads 数组是图片上传组，每组独立一个九宫格。
 */
import { STATIC_BASE } from '@/shared/config'

export type FieldType = 'text' | 'phone' | 'number' | 'select' | 'region' | 'textarea'
...
export interface ServiceField {
  ...
  /** number 型右侧显示的单位 */
  unit?: string
  /** select 型的选项。留空表示运行时生成（目前只有购买年份） */
...
export interface Service {
  ...
  uploads: readonly UploadGroup[]
}

/**
 * 需求原文里，后三个服务（买手 / 售后 / 流转）没有列联系方式字段。
 * 没有联系方式运营没法跟进，所以这三个也补了「客户名称 + 联系方式」。
 */
const CONTACT_FIELDS: readonly ServiceField[] = [
  {
    id: 'name',
    label: '客户名称',
    type: 'text',
...
const PROJECT_TYPES = ['住宅项目', '商业项目', '办公项目', '其他项目'] as const
...
export const services: readonly Service[] = [
...

  {
    ...
    fields: [
      ...
      },
      {
        ...
        label: '是否接受二手/中古',
        type: 'select',
        options: ['接受', '不接受', '视品相而定'],
      },
      {
        id: 'style',
        label: '意向风格',
        type: 'text',
        placeholder: '如 中古、包豪斯、侘寂',
        maxlength: 60,
      },
      ...
      { id: 'budget', label: '购买预算', type: 'number', unit: '元' },
      {
        ...
        label: '可接受的最长寻购期',
        type: 'select',
        options: ['1 个月内', '3 个月内', '半年内', '一年内', '不限'],
      },
      {
        id: 'note',
        label: '其他备注说明',
        type: 'textarea',
        placeholder: '尺寸、颜色、使用场景等',
        maxlength: 500,
      },
    ],
...

  {
    ...
    fields: [
      ...
      // options 留空：年份按当前年在 ApplyPage 里生成，写死会年年过期
      { id: 'buyYear', label: '商品购买年份', type: 'select', options: [] },
      {
        id: 'demand',
        label: '售后需求',
        type: 'select',
        options: ['安装指导', '运输破损', '维修修复', '清洁保养', '使用咨询', '其他'],
      },
      { id: 'quantity', label: '数量', type: 'number', unit: '件' },
...

  {
    ...
    fields: [
      ...
      { id: 'buyYear', label: '商品购买年份', type: 'select', options: [] },
      {
        ...
        label: '商品新旧程度',
        type: 'select',
        options: ['全新未拆封', '九成新', '八成新', '七成新', '六成新及以下'],
      },
      { id: 'quantity', label: '数量', type: 'number', unit: '件' },
      { id: 'city', label: '商品所在城市', type: 'region' },
...
export function findService(id: string | undefined): Service | null {
  return services.find((s) => s.id === id) ?? null
}

/** 购买年份选项：从今年往回数 20 年。小程序 apply.js 也是这么生成的 */
export function yearOptions(): string[] {
  const now = new Date().getFullYear()
  return Array.from({ length: 20 }, (_, i) => `${now - i} 年`)
}

/** 每个上传组最多几张，与小程序 apply.js 的 MAX_PER_GROUP 一致 */
